@extends('admin.admin-layout')

@section('title', 'Admin List')

@section('content')



    <main id="main" class="main">
        <!-- Page Title -->
        <div class="pagetitle">
            <h1>Admin List - {{ $data->total() }}</h1>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item action">Home</li>
                    <li class="breadcrumb-item action">Account</li>
                    <li class="breadcrumb-item action">Admin List</li>
                </ol>
            </nav>
        </div>

        <!-- Admin Create Success Message -->
        <div class="col-lg-6 offset-lg-3">
            @if (session('success'))
                <div class="alert alert-success alert-dismissible fade show" role="alert">
                    <i class="fa-solid fa-square-check me-2"></i> <strong>{{ session('success') }}</strong>
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                </div>
            @endif
        </div>

        @if (count($data) == 0)
            <h4 class="text-center mt-4 font-bold">There is no <span class="text-danger">Admin Data!</span></h4>
        @else
            <!--Admin List -->
            <div class="container-fluid mt-5">
                <table class="table table-responsive table-hover align-middle ">
                    <thead>
                        <tr>
                            <th></th>

                            <th>Id</th>
                            <th>Name</th>
                            <th>Email</th>
                            <th>Join Date</th>
                            <th></th>

                        </tr>
                    </thead>

                    <tbody>
                        @foreach ($data as $admin)
                            <tr>

                                <td class="col-lg-1">
                                    <a href="{{ route('admin.detail', $admin->id) }}"><button class="btn btn-outline-info"
                                            title="detail admin account"><i
                                                class="fa-solid fa-circle-info"></i></button></a>
                                </td>

                                <td class="col-lg-1"><a href="{{ route('admin.detail', $admin->id) }}" title="Admin detail"
                                        class="fw-bolder btn-link ">
                                        <h5>{{ $admin->id }}</h5>
                                    </a></td>
                                <td class="col-lg-2">
                                    <div class="input-group d-flex align-items-center justify-content-start gap-1">
                                        <i class="fa-solid fa-user me-2"></i>{{ $admin->name }}
                                    </div>
                                </td>
                                <td class="col-lg-3"><i class="fa-solid fa-envelope me-2"></i>{{ $admin->email }}</td>

                                <td class="col-lg-3"><i
                                        class="fa-solid fa-calendar-days text-success me-2"></i>{{ $admin->created_at->format('j / F / Y') }}
                                </td>

                                <td class="col-lg-1">
                                    @if (Auth::user()->id != $admin->id)
                                        <a href="{{route('admin.delete',$admin->id)}}">
                                            <button class="btn btn-danger" title="delete admin account">
                                                <i class="fa-solid fa-delete-left"></i>
                                            </button>
                                        </a>
                                    @endif
                                </td>
                            </tr>
                        @endforeach

                    </tbody>

                </table>
                <div class="mt-5">
                    {{ $data->links() }}
                </div>
            </div>
        @endif


    </main>
@endsection
